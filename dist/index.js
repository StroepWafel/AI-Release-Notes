 ...
 */
function parseTagWithChannel(tag) {
...
function getSameChannelTags(candidates, currentTag) {
 ...
 */
function reassembleDiff(chunks) {
    return chunks
...
async function getPreviousTag(octokit, owner, repo, currentTag, previousTag) {
    try {
        ...
        const commitSha = (0, child_process_1.execSync)(`git rev-parse "${tagToUse}"`, { encoding: 'utf-8' }).trim();
 ...
 */
function getRawDiff(previousCommit, currentCommit) {
    if (!previousCommit)
        return '';
    try {
        return (0, child_process_1.execSync)(`git diff ${previousCommit}..${currentCommit}`, {
            encoding: 'utf-8',
 ...
 */
function getFullDiff(previousCommit, currentTag, lineLimit) {
    try {
        let currentCommit;
        try {
            currentCommit = (0, child_process_1.execSync)(`git rev-parse ${currentTag}`, { encoding: 'utf-8' }).trim();
        ...
        let fullDiff;
        try {
            fullDiff = (0, child_process_1.execSync)(`git diff ${previousCommit}..${currentCommit}`, {
                encoding: 'utf-8',
                maxBuffer: 2 * 1024 * 1024
            });
        }
 ...
 */
function appendDiffSection(body, previousTag, currentTag, previousCommit, diffSectionLimit, owner, repo, includeInlineDiff, notesStyle) {
    ...
    const lines = isDesqta ? ['', ''] : ['', '', '## Changes (diff)', ''];
    lines.push(isDesqta
 ...
 */
function parsePerFileDiffs(fullDiff) {
    const chunks = [];
 ...
 */
async function summarizeFileChanges(groqClient, model, filePath, fileDiff) {
 ...
 */
async function summarizeAllFiles(groqClient, model, fileDiffs) {
    const CONCURRENCY = 3;
    const results = [];
    for (let i = 0; i < fileDiffs.length; i += CONCURRENCY) {
        const batch = fileDiffs.slice(i, i + CONCURRENCY);
        const batchResults = await Promise.all(batch.map(({ path, diff }) => summarizeFileChanges(groqClient, model, path, diff)));
...
function getChangedFiles(previousCommit, currentTag) {
    try {
        // Get the commit SHA that the current tag points to
        let currentCommit;
        try {
        ...
        if (!previousCommit) {
            ...
            return (0, child_process_1.execSync)(`git diff --name-only ${currentCommit}`, { encoding: 'utf-8' });
        }
        // Get changed files between commits
        return (0, child_process_1.execSync)(`git diff --name-only ${previousCommit}..${currentCommit}`, { encoding: 'utf-8' });
    }
    catch (error) {
...
Generate the release notes now:`;
    try {
        ...
        let suggestedReleaseTitle = null;
...
async function run() {
    try {
        ...
        let currentCommit;
        try {
            currentCommit = (0, child_process_1.execSync)(`git rev-parse ${tagName}`, { encoding: 'utf-8' }).trim();
        }
        catch {
            currentCommit = (0, child_process_1.execSync)('git rev-parse HEAD', { encoding: 'utf-8' }).trim();
        }
        ...
        const finalBody = appendDiffSection(releaseNotes, previousTag, tagName, previousCommit, diffSectionLimit, owner, repo, showDiffSection, notesStyle) + RELEASE_NOTES_ATTRIBUTION;
